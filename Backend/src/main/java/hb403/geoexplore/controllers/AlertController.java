...
public class AlertController {
	...
	@Operation(summary = "Get a list of the alerts whose locations are bounded by the provided WKT geometry string")
	@GetMapping(path = "geomap/alerts/within")
	public @ResponseBody List<AlertMarker> getEventsWithinBounds(@RequestBody String wkt_bounds_geom) {	// takes in 'well known text' for the bounding geometry --> may define special json formats for predefined bounds later
		try {
			final List<AlertMarker> bounded = this.alert_repo.findWithin( GeometryUtil.getGeometry(wkt_bounds_geom) );
			...
			System.out.println("AlertMarker.getEventsWithinBounds(): Encountered exception! -- " + e.getMessage());
