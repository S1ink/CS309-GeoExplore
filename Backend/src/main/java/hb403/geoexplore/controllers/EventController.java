...
public class EventController {
	...
	public @ResponseBody List<EventEntity.JsonFormat> getEventsWithinBounds(@RequestBody String wkt_bounds_geom) {	// takes in 'well known text' for the bounding geometry --> may define special json formats for predefined bounds later
		try {
			final List<EventEntity> bounded = this.db_events.findWithin( GeometryUtil.getGeometry(wkt_bounds_geom) );
			System.out.println("Recieved " + bounded.size() + " bounded events");
			final ArrayList<EventEntity.JsonFormat> formatted = new ArrayList<>();
			for(EventEntity v : bounded) {
				formatted.add(EventEntity.formatJson(v));
			}
			return formatted;
		} catch(Exception e) {
			System.out.println("EventEntity.getEventsWithinBounds(): Encountered exception! -- " + e.getMessage());
			// continue >>> (return null)
		}
		return null;
	}
