...
public class ObservationController {
	...
	public @ResponseBody Set<ObservationMarker> getObservationsWithinPoly(@RequestBody String wkt_bounds_geom) {	// takes in 'well known text' for the bounding geometry --> may define special json formats for predefined bounds later
		try {
			final Set<ObservationMarker> bounded = this.obs_repo.findWithin( GeometryUtil.getGeometry(wkt_bounds_geom) );
			...
			System.out.println("ObservationMarker.getObservationsWithinPoly(): Encountered exception! -- " + e.getMessage());
			// continue >>> (return null)
		}
		return null;
	}
	...
	@GetMapping(path = "geomap/observations/within/rect")
	public @ResponseBody Set<ObservationMarker> getObservationsWithinBounds(@RequestBody LocationRange range) {	// takes in 'well known text' for the bounding geometry --> may define special json formats for predefined bounds later
        if(range == null || range.isInvalid()) return null;
        try {
			final Set<ObservationMarker> bounded = this.obs_repo.findWithin( range.getRect() );
			// System.out.println("Recieved " + bounded.size() + " bounded events");
			for(ObservationMarker o : bounded) {
				o.enforceLocationTable();
			}
